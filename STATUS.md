...
> **2026-08-07 칸 순서 교체**: 행 순서를 `추임새 | 한국어 | 영어` → **`추임새 | 영어(희미) | 한국어`**로 변경.
> 이유: 한국어가 가운데 있으면 원문 recall 전에 번역을 경유하게 됨(Pat 진단) — 추임새를 보고
> **영어 원문을 직접 떠올리는 회상 과제**로 전환, 한국어는 오른쪽 "의미 확인" 수단으로 강등
> (13px·회색으로 시각 위계도 낮춤). 안내 문구·헤더 라벨·JSDoc 갱신, 추임새 없는 2칸 모드도
> `영어 | 한국어` 순서. gloss 계약·캐시 무변경. vitest 14/14, lib("use client" 확인)·데모 빌드 통과.
